...
```asm
irq_handler:
    ldr sp, =_stack_start    @ 自初始化本模式的 SP（各异常模式 banked SP 互不相同）
...
**为什么第一条必须是 `ldr sp, =_stack_start`？** 每种异常模式都有独立的 banked SP，reset 时未被统一初始化。若不在 handler 入口处自行初始化，`push {r0-r12, lr}` 会向随机地址写数据，立即崩溃。
...
```rust
global_asm!(
    // 异常处理函数（每个 handler 首先自初始化本模式的 SP）
...
> **重要：HYP 模式切换**
>
> mps3-an536 上电时 CPU 处于 **HYP 模式**（AArch32 EL2）。HYP 模式有独立的向量表基址（HVBAR），与我们设置的普通向量表（VBAR）无关。如果不切换到 SVC 模式，FIQ/IRQ 触发时 CPU 会跳到 HVBAR + offset，而不是 `_vectors + offset`，导致中断永远无法到达我们写的 handler。
>
> `eret` 指令以原子方式把 CPU 从 HYP 切换到 SVC 模式（SPSR_hyp → CPSR），并跳转到 ELR_hyp 指定的地址。
